...
s.t. Control_2{k in 1..N_Colab-1}: TPC[k]>=Tmin;
s.t. Contro2_0: Tmin=TPC[N_Colab];
s.t. Contro2_1{k in K}: TPC[k]<=Tmax;
s.t. Control_3{k in K}: Tmax<= PCT;
s.t. Contro2_4{k in K}: TPC[k]>=Tmin;
##a.t  Control_5 Tmin >=-Mi(1-yi(k))+TPC[k];
##somatorio yi[k]=1;


##i in 2..N.open

##x i,j,k=xi+j,k+Troca[i]
#TPC[i]=TPC[i+1]
